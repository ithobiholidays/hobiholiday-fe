...
const AdminTestimonialView = () => {
      ...
      render: (row) => (
        <div className="flex gap-2">
          <button
            ...
            onClick={() => router.push(`/dashboard/testimonials/${row?.id}`)}
          >
            <MdOutlineEdit />
            Edit
          </button>
  ...
  const fetchData = async () => {
    try {
      await postData(`/testi/alls?q=${searchValue}`, {
        p: page,
        limit: perPage,
      });
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex gap-10 items-center">
        <p className="text-3xl font-medium">Testimonial</p>
